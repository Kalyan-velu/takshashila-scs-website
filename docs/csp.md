...
- **`frame-src`** — `'self'` + Turnstile challenge iframe + `www.googletagmanager.com` (GTM's `<noscript>` fallback iframe, `src/lib/GTag/GoogleTagManagerNoscript.astro`) + `td.doubleclick.net` (Google Ads conversion linker's cross-domain iframe fallback).
