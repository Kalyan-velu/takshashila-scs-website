...
- **`script-src`** — `'self'` + `'unsafe-inline'` + `challenges.cloudflare.com` + `code.iconify.design` + `www.googletagmanager.com` + `www.clarity.ms` + `scripts.clarity.ms` + `googleads.g.doubleclick.net`. The GTM/Clarity/Ads origins are needed even though those scripts load inside the Partytown web worker: Partytown fetches the actual script bytes through the main thread's network stack (via its sandboxed proxy), so the browser still enforces `script-src` against the real script origin. `scripts.clarity.ms` is the actual CDN the "Microsoft Clarity - Official" GTM tag loads its runtime from (Clarity's classic docs reference `www.clarity.ms/tag/...`, but the current official tag template pulls from `scripts.clarity.ms`) — both are kept since either can appear depending on the Clarity snippet version. `googleads.g.doubleclick.net` specifically is the Google Ads conversion tracking tag (`AW-16482556669`, configured inside the GTM container) loading its `viewthroughconversion` pixel script.
...
- **`connect-src`** — `'self'` + Cloudflare + Iconify API endpoints (`api.iconify.design`, `api.simplesvg.com`, `api.unisvg.com`) + GTM/GA/Clarity/Ads beacon endpoints: `www.googletagmanager.com`, `www.google-analytics.com`, `region1.google-analytics.com`, `analytics.google.com` (GA4 `/g/collect`), `www.clarity.ms`, `c.clarity.ms`, `scripts.clarity.ms`, `www.google.com` (`/g/collect`, `/ccm/collect`, `/rmkt/collect` — GA4 + Google Ads remarketing), `ad.doubleclick.net` (`/ccm/s/collect`), `stats.g.doubleclick.net` (`/g/collect`), `googleads.g.doubleclick.net` (Ads conversion beacon). All of these come from the GTM container's own tags (GA4, Google Ads conversion/remarketing) firing at runtime — not from this codebase directly — so the list tracks whatever's configured in Tag Manager rather than anything in `src/`. Blog posts are fetched from Blogger's public JSON feed (`src/lib/blogger.ts`), but only from Astro frontmatter at build/request time on the server — never from browser JS — so `blogs.takshashilascs.com` doesn't need to be listed here.
