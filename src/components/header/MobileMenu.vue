    ...
    <div class="w-full px-2 pt-8">
      <div class="w-full">
        ...
        <ul class="flex flex-col items-start space-y-4 w-full">
          <li class="relative w-full border-b border-border/40 pb-2">
            <button
              @click="toggleAccordion('courses')"
              class="w-full flex justify-between items-center text-left link text-4xl tracking-tight leading-none transition-none dark:text-white pb-2"
            >
              <span>UPSC - APSC</span>
              <svg
                class="w-6 h-6 transform transition-transform"
                :class="{ 'rotate-180': openAccordion === 'courses' }"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            ...
            <div
              ...
              :style="{
                maxHeight: openAccordion === 'courses' ? '500px' : '0px',
                opacity: openAccordion === 'courses' ? 1 : 0,
              }"
            >
              <div
                class="mt-2 mb-4 flex flex-col space-y-3 pl-4 border-l-2 border-primary/20"
              >
                <a
                  v-for="item in coursesItems"
                  :key="item.url"
                  :href="item.url"
                  @click="emit('closeMenu')"
                  class="text-lg font-light text-foreground/80 dark:text-gray-300 hover:text-primary transition-colors"
                  >{{ item.title }}</a
                >
                <a
                  href="/courses"
          ...
          <li class="relative w-full border-b border-border/40 pb-2">
            ...
            >
              ...
              <svg
                class="w-6 h-6 transform transition-transform"
                :class="{ 'rotate-180': openAccordion === 'about' }"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <div
              class="overflow-hidden transition-all duration-300"
              :style="{
                maxHeight: openAccordion === 'about' ? '500px' : '0px',
                opacity: openAccordion === 'about' ? 1 : 0,
              }"
            >
              <div
                class="mt-2 mb-4 flex flex-col space-y-3 pl-4 border-l-2 border-primary/20"
      ...
      <div class="w-full mt-8 pt-2">
        ...
        <div class="flex items-center space-x-4 mb-8 px-2">
          <a
            href="#"
            class="w-10 h-10 rounded-full border border-border flex items-center justify-center text-foreground/60 hover:text-primary transition-colors"
            ><svg
              class="w-4 h-4 fill-current"
              viewBox="0 0 320 512"
              xmlns="http://www.w3.org/2000/svg"
