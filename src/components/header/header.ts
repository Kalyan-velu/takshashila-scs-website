...
/* ==========================================================================
   Elements
   ========================================================================== */
const headerContainer = document.querySelector(".js-header-container") as HTMLElement;
...
const logoOverlay     = document.querySelector(".js-logo-scroll-overlay") as HTMLElement;
const logos           = document.querySelectorAll(".js-header-logo")  as NodeListOf<HTMLElement>;

const coursesMenu  = document.querySelector(".js-mega-menu-courses") as HTMLElement;
const aboutMenu    = document.querySelector(".js-mega-menu-about")   as HTMLElement;
const searchPanel  = document.querySelector(".js-search-dropdown")   as HTMLElement;

const triggerCourses = document.querySelector(".js-menu-trigger-courses") as HTMLElement;
const triggerAbout   = document.querySelector(".js-menu-trigger-about")   as HTMLElement;
const triggerDirects = document.querySelectorAll(".js-menu-trigger-direct") as NodeListOf<HTMLElement>;

const chevronCourses = document.querySelector(".js-chevron-courses") as HTMLElement;
...
const searchIconOpen  = document.querySelector(".js-search-icon-search") as HTMLElement;
const searchIconClose = document.querySelector(".js-search-icon-close")  as HTMLElement;
...
if (headerContainer && ticker && topBar && header && logos.length > 0) {
  ...
  headerTimeline
    ...
    .to(header,  { paddingBlock: window.innerWidth < 768 ? "0.5rem" : "0.75rem", ease: "power1.inOut" }, 0);

  logos.forEach(logo =>
    headerTimeline?.to(logo, { width: window.innerWidth < 768 ? "9rem" : "12rem", ease: "power1.inOut" }, 0)
  );
  ...
  handleScroll = () => {
    ...
    if (st > lastScrollY && st > 300) {
      gsap.to(headerContainer, { yPercent: -100, duration: 0.35, ease: "power2.out" });
      closeMegaMenus();
      document.dispatchEvent(new CustomEvent("header:close-search"));
      document.dispatchEvent(new CustomEvent("header:close-mobile"));
    } else {
      gsap.to(headerContainer, { yPercent: 0, duration: 0.35, ease: "power2.out" });
    }
...
function openMenu(el: HTMLElement) {
  ...
  activeMenu = el;
  if (el === coursesMenu) chevronCourses?.classList.add("rotate-180", "text-primary-500");
  if (el === aboutMenu)   chevronAbout?.classList.add("rotate-180", "text-primary-500");
}
...
// Hover open
triggerCourses?.addEventListener("mouseenter", () => coursesMenu && openMenu(coursesMenu));
triggerAbout?.addEventListener("mouseenter",   () => aboutMenu   && openMenu(aboutMenu));
...
triggerDirects.forEach(link => link.addEventListener("mouseenter", closeMegaMenus));

// Close when a link inside a mega menu is clicked
document.querySelectorAll(".js-mega-link").forEach(link =>
...
function openSearch() {
  ...
  searchPanel?.classList.remove("opacity-0", "-translate-y-4", "pointer-events-none");
  searchPanel?.classList.add("opacity-100", "translate-y-0", "pointer-events-auto");
  searchIconOpen?.classList.add("hidden");
  searchIconClose?.classList.remove("hidden");
  searchIconClose?.classList.add("block");
  searchToggle?.classList.remove("bg-slate-900", "dark:bg-white", "dark:text-slate-900");
...
function closeSearch() {
  searchOpen = false;
  searchPanel?.classList.remove("opacity-100", "translate-y-0", "pointer-events-auto");
  searchPanel?.classList.add("opacity-0", "-translate-y-4", "pointer-events-none");
  ...
  searchToggle?.classList.add("bg-slate-900", "dark:bg-white", "dark:text-slate-900");
